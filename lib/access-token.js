...
export default class AccessToken {
  ...
  expired(expirationWindowSeconds = 0) {
    const expiresInPropertyName = this.#config.options.expiresInPropertyName || "expires_at";
    return this.token[expiresInPropertyName]
      ? this.token[expiresInPropertyName] - (Date.now() + expirationWindowSeconds * 1000) <= 0
      : false;
  }
  ...
  async refresh(params = {}, httpOptions = {}, tokenExpiryDuration) {
    ...
    return new AccessToken(this.#config, this.#client, {
      ...response,
      expires_in: response.expires_in || tokenExpiryDuration,
    });
  }
