...
module.exports = class KekaApi {
  ...
  async getToken(params, httpOptions) {
    const parameters = GrantTypeParams.forGrantType('kekaapi', this.#config.options, params);
    const response = await this.#client.request(this.#config.auth.tokenPath, {
      ...parameters.toObject(),
