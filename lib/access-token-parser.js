...
export default function parseToken(token) {
  ...
  if (EXPIRES_AT_PROPERTY_NAME in token && token[EXPIRES_AT_PROPERTY_NAME]) {
    ...
    REFRESH_TOKEN_EXPIRES_IN_PROPERTY_NAME in token &&
    parseInt(token[REFRESH_TOKEN_EXPIRES_IN_PROPERTY_NAME], 10)
  ) {
    tokenProperties[REFRESH_TOKEN_EXPIRES_IN_PROPERTY_NAME] = getExpirationDate(
      token[REFRESH_TOKEN_EXPIRES_IN_PROPERTY_NAME],
      token[CREATION_DATE_PROPERTY_NAME]
    );
  } else {
