...
module.exports = class Client {
  ...
  async request(url, params, opts) {
    ...
    const options = Hoek.applyToDefaults(this.#httpOptions, requestOptions.toObject(opts));

    debug('Creating request to: (POST) %s', url);
    debug('Using request options: %j', options);

    const response = await fetch(new URL(options.url || url, this.#httpOptions.baseUrl),
