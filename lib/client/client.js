...
class Client {
  ...
  async request(url, params, opts) {
    ...
    const response = await pRetry(
      async () => {
        ...
        if (!response.ok) {
          ...
          error.originalError.status = response.status;
          error.originalError.headers = headersObj;
          error.originalError.request_payload = options.payload;
          error.originalError.request_headers = options.headers;
          error.originalError.request_url = requestUrl;
          error.originalError.response = clonedResponse;
          throw error;
        }
    ...
    try {
      ...
      error.originalError.request_payload = options.payload;
      error.originalError.request_headers = options.headers;
      error.originalError.request_url = options.url;
      error.originalError.response = clonedResponse;
      throw error;
    }
  }
