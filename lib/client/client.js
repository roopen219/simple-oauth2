...
module.exports = class Client {
  ...
  async request(url, params, opts) {
    ...
    const response = await fetch(
      new URL(options.url || url, this.#httpOptions.baseUrl).href,
      { method: 'post', headers: options.headers, body: options.payload },
    );

    if (!response.ok) {
      const bodyText = await response.text();
      throw new Error(`Failed to create access token: ${response.status} ${response.statusText} ${bodyText}`);
    }
    ...
    try {
      ...
      throw new Error(`Failed to parse response body as JSON: ${bodyText}`);
    }
  }
};
